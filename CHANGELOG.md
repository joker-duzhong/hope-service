...
- 新增班主任工作台（Teacher Logbook）后端应用：班级与学生管理、独立台账资源 CRUD、座位版本并发控制、首页与收支/培训汇总、学生 CSV 导入导出、UI 偏好及同步备份恢复。
- 新增 `0018_teacher_logbook` 数据库迁移，所有业务表采用 `teacher_logbook_` 前缀、用户与班级隔离、软删除及关键条件唯一索引。

## 2026-07-16

- 新增账伴（Ledger Mate）P0 后端模块：默认账本、分类和支付方式初始化，按用户隔离的账单 CRUD 与幂等提交、操作审计、基础检索及统计，以及 AI 待确认账单入账接口。
- 应用账伴初始数据库迁移 `0016_ledger_mate_initial`，修复账伴分类与支付方式接口因 `ledger_mate_books` 表不存在而返回 500 的问题。
- 修复账伴首屏并发加载分类与支付方式时重复初始化默认账本，触发唯一约束并返回 500 的问题。
- 新增账伴 AI 会话记账：会话与消息历史、近 5 条上下文解析、最多 100 条历史读取、结构化账单自动入账及消息-账单实时引用。
